...
def create_stock(payload: StockCreate, db: Session = Depends(get_db), current_manager=Depends(get_current_manager)):
    ...
    return stock_service.serialize_stock_read(stock)
...
def list_stocks(db: Session = Depends(get_db), current_manager=Depends(get_current_manager)):
    ...
    return [stock_service.serialize_stock_read(stock) for stock in stocks]
...
@router.delete("/{stock_id}", status_code=204, summary="Delete an empty stock row.")
def delete_stock(stock_id: UUID, db: Session = Depends(get_db), current_manager=Depends(get_current_manager)):
    stock_service.delete_stock(db, current_manager, stock_id)
