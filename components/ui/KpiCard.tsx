...
export function KpiCard({
  ...
  return (
    <article className="premium-card reveal rounded-2xl p-4 sm:p-5" style={{ ["--delay" as string]: delay ?? "0ms" }}>
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs uppercase tracking-wide text-[var(--muted)]">{label}</p>
        <span className={`h-2.5 w-2.5 rounded-full ${dotClass}`} aria-hidden />
      </div>
      <p className="mt-2 text-2xl font-semibold text-[var(--text)] sm:text-3xl">
